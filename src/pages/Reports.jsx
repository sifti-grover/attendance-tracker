...
export default function Reports() {
  ...
  async function loadSessions() {
    const { data } = await supabase.from("sessions").select("id, session_name").order("created_at", { ascending: false });
    setSessions(data || []);
  }
  ...
  async function loadAttendance(id) {
    const { data } = await supabase
      .from("attendance")
      .select("id, attendance_status, scanned_at, students:student_id(student_name,roll_no,student_email)")
      .eq("session_id", id)
  ...
  const csv = useMemo(() => {
    ...
    const body = rows.map((r) => [r.students?.student_name || "", r.students?.roll_no || "", r.students?.student_email || "", r.attendance_status, r.scanned_at || ""]); 
    return [header, ...body].map((line) => line.map((x) => `"${(x || '').toString().replaceAll('"', '""')}"`).join(",")).join("\n");
  }, [rows]);
  ...
  return (
    <div className="space-y-4">
      <div className="bg-white shadow rounded p-4 flex items-center gap-3">
        <select className="border rounded px-3 py-2" value={sessionId} onChange={(e) => setSessionId(e.target.value)}>
        ...
        <div className="ml-auto text-sm">
          <span className="mr-4">Present: <b>{counts.present}</b></span>
          <span>Absent: <b>{counts.absent}</b></span>
        </div>
        <button disabled={!sessionId} onClick={downloadCsv} className="border rounded px-3 py-2 disabled:opacity-60">Download CSV</button>
      </div>
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.id} className="border-b">
                ...
                <td className="py-2 pr-4">{r.scanned_at ? new Date(r.scanned_at).toLocaleString() : ""}</td>
              </tr>
            ))}
            {rows.length === 0 && (
